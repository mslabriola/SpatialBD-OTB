---
title: "Fishing trawlers drive the fine-scale distribution of bottlenose dolphins
  at the Tiber River estuary (Western Mediterranean Sea)"
author: "Labriola et al."
date: "`r Sys.Date()`"
output:
  html_document:
    ...
    toc_float: true 
    code_folding: hide
    number_sections: true  # Numbered section headers
...
Here we explore the spatial and temporal correlation between bottlenose dolphins (Tursiops truncatus, hereafter BD) and trawling fishery in the Tiber River estuary area. 

Specifically, we aimed to determine a) whether trawling significantly influenced BD spatial distribution, and b) whether BD exhibited preferences for specific trawlers or areas. 
...
![Dolphins interacting with trawling fishery](images/interaction_Tt_trawl.jpg)
...
# Computed centroids of sighting tracks
...
```{r RO, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r biv_maps, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r cdf, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r overlap_j, echo=FALSE, fig.height=15/2.54, fig.width=17/2.54, message=FALSE, warning=FALSE, dpi=330}
...
```{r glmm, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r vsplot_GLMM, echo=FALSE, message=FALSE, warning=FALSE, dpi=330}
...
```{r catch, echo=TRUE, message=FALSE, warning=FALSE}
